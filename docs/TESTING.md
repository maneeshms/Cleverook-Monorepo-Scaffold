...
```bash
pnpm run test                 # all projects, coverage enforced
pnpm exec nx test api              # one project
pnpm exec nx test api --watch      # iterate
pnpm exec nx test api --coverage   # per-file report
```
...
```bash
pnpm run db:up          # Postgres + Redis
pnpm run e2e:setup      # create + migrate clevscaffold_test
pnpm run e2e            # run all e2e suites (--parallel=1)
```
...
- **Runtime scanner:** `pnpm run scan:security` against a live api
  (`pnpm exec nx serve api` first). 49 black-box checks, all passing at baseline; exits
...
pnpm run e2e     # when logic or endpoints changed
```
