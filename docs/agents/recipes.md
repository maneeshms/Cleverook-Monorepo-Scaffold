...
## Enable a capability in a generated project (add.mjs)

Applies in projects generated by init (they have `.clevscaffold.json`, no
`scripts/init.mjs`). **Never hand-copy scaffold code** — the tool copies and
re-registers everything mechanical; you only apply the wiring guide.

1. `node scripts/add.mjs <capability> [--from ../ClevScaffold]` — capabilities:
   `auth`, `messaging`, `feature-flags`, `metrics`, `compliance` (requires are
   auto-added, e.g. messaging pulls in auth).
2. Open `docs/wiring-<capability>.md` (generated). Apply each section to the
...
`.clevscaffold.json` lists the capability (the tool updates it).

## Create a new app (new-app.mjs)

1. `node scripts/new-app.mjs --type api|vite|next|expo --name <kebab> [--port <n>]`
   (in a generated project add `--from`/rely on the recorded origin; in the
   scaffold repo it uses the repo itself).
