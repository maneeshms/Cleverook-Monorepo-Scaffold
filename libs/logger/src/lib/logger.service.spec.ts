...
describe('LoggerService', () => {
  ...
  let winston: { info: jest.Mock; warn: jest.Mock; error: jest.Mock; debug: jest.Mock; verbose: jest.Mock };

  beforeEach(() => {
    winston = { info: jest.fn(), warn: jest.fn(), error: jest.fn(), debug: jest.fn(), verbose: jest.fn() };
  ...
  it('writes a success audit at info with category=audit', () => {
    ...
    expect(winston.info).toHaveBeenCalledWith(
      'AUDIT: LOGIN',
      expect.objectContaining({ category: 'audit', context: 'AuditLog', status: AuditStatus.SUCCESS }),
    );
  });

  it('writes a failure audit at warn', () => {
    service.auditAuth(AuditAction.LOGIN, AuditStatus.FAILURE, 'u1', { reason: 'bad_password' });
    expect(winston.warn).toHaveBeenCalledWith('AUDIT: LOGIN', expect.objectContaining({ category: 'audit' }));
  });
